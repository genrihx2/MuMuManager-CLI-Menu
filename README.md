...
- Все сетевые операции — только HTTPS к `api.github.com` (проверка версий, загрузка обновлений, валидация токена) и `raw.githubusercontent.com` (загрузка файлов обновления); стартовая проверка — read-only, загрузка только по явному выбору `[U]` с подтверждением
